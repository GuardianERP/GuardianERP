...
function StatCard({ title, value, change, changeType, icon: Icon, color, onClick }) {
  return (
    <div 
      className="card p-3 sm:p-4 md:p-6 hover:shadow-lg transition-all cursor-pointer active:scale-[0.98]"
      onClick={onClick}
    >
      <div className="flex items-start justify-between">
        <div className="min-w-0 flex-1">
          <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 truncate">{title}</p>
          <p className="text-xl sm:text-2xl md:text-3xl font-bold text-gray-900 dark:text-white mt-1 truncate">
            {value}
          </p>
          {change && (
        ...
        <div className={`w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 rounded-lg md:rounded-xl flex items-center justify-center flex-shrink-0 ml-2 ${color}`}>
...
function QuickAction({ title, description, icon: Icon, onClick, color }) {
  return (
    <button 
      className="card p-3 sm:p-4 text-left hover:shadow-lg transition-all w-full group active:scale-[0.98]"
      onClick={onClick}
    >
      <div className="flex items-center gap-3 sm:gap-4">
        <div className={`w-9 h-9 sm:w-10 sm:h-10 rounded-lg flex items-center justify-center flex-shrink-0 ${color}`}>
          <Icon className="w-4 h-4 sm:w-5 sm:h-5 text-white" />
        </div>
        <div className="flex-1 min-w-0">
          <p className="font-medium text-sm sm:text-base text-gray-900 dark:text-white truncate">{title}</p>
          <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 truncate">{description}</p>
        </div>
        <ArrowRight className="w-4 h-4 sm:w-5 sm:h-5 text-gray-400 group-hover:text-blue-500 group-hover:translate-x-1 transition-all flex-shrink-0" />
      </div>
    </button>
  );
}
...
function DashboardPage() {
  ...
  return (
    <div className="space-y-4 sm:space-y-6 animate-fade-in">
      {/* Welcome Banner */}
      <div className="card bg-gradient-to-r from-blue-600 to-blue-700 p-4 sm:p-6 text-white">
          ...
          <div className="flex items-center gap-2 sm:gap-4 flex-shrink-0">
            ...
            <div className="hidden md:block">
              <div className="text-right">
                <p className="text-3xl lg:text-4xl font-bold">{new Date().getDate()}</p>
